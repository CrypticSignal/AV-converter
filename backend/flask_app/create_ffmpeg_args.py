...
def get_ffmpeg_args(chosen_codec, input_filename, output_name, is_keep_video, data, slider_value):
    # AAC
    if chosen_codec == "AAC":
        return aac(
            input_filename,
    ...
    # AC3
        ...
        return ac3(input_filename, output_name, is_keep_video, data["ac3Bitrate"])
    # ALAC
    elif chosen_codec == "ALAC":
        return alac(input_filename, output_name, is_keep_video)
    # CAF
    elif chosen_codec == "CAF":
        return caf(input_filename, output_name)
    # DTS
    elif chosen_codec == "DTS":
        return dts(input_filename, output_name, is_keep_video, data["dtsBitrate"])
    # FLAC
    elif chosen_codec == "FLAC":
        return flac(input_filename, output_name, is_keep_video, data["flacCompression"])
    # MKA
    elif chosen_codec == "MKA":
        return mka(input_filename, output_name)
    # MKV
    elif chosen_codec == "MKV":
        return mkv(input_filename, output_name, data["videoSetting"], data["crfValue"])
    # MP3
    ...
    # MP4
        ...
        return mp4(input_filename, output_name, data["videoSetting"], data["crfValue"])
    # Opus
    elif chosen_codec == "Opus":
        return opus(input_filename, output_name, data["opusEncodingType"], slider_value)
    # Vorbis
    elif chosen_codec == "Vorbis":
        return vorbis(
            input_filename, output_name, data["vorbisEncodingType"], data["qValue"], slider_value
        )
    # WAV
    elif chosen_codec == "WAV":
        return wav(input_filename, output_name, is_keep_video, data["wavBitDepth"])


def return_args(input_filename, encoding_args, output_name):
...
# ALAC
def alac(input_filename, output_name, is_keep_video):
    if is_keep_video == "yes":
        return return_args(
            input_filename,
            "-c:v copy -c:a alac",
...
# CAF
def caf(input_filename, output_name):
    return return_args(input_filename, "-c:a alac", f"{output_name}.caf")
...
# FLAC
def flac(input_filename, output_name, is_keep_video, flac_compression):
    if is_keep_video == "yes":
        return return_args(
            input_filename,
...
# MKA
def mka(input_filename, output_name):
    return return_args(
        input_filename,
        "-map 0:a -c:a copy",
        f"{output_name}.mka",
...
# MKV
def mkv(input_filename, output_name, video_mode, crf_value):
...
# MP4
def mp4(input_filename, output_name, video_mode, crf_value):
...
# Opus
def opus(input_filename, output_name, encoding_type, bitrate):
...
# WAV
def wav(input_filename, output_name, is_keep_video, wav_bit_depth):
    if is_keep_video == "yes":
        return return_args(
            input_filename,
