...
# Create a function that will read the progress file generated by FFmpeg.
def ReadProgress():
    while True:
        with open('progress.txt', 'r') as f:
            lines = f.readlines()
            time = lines[-5].split('=')[-1].split('.')[0]
            progress_message = time + " [HH:MM:SS]" + " of the file has been converted so far..."
            # Trigger a new event calledg "show progress" 
    ...
    if request.form["requestType"] == "convert":
        ...
        try:
            if not progress_thread.is_alive():
                ...
                app.logger.info("Thread wasn't running. Started.")
            else:
                app.logger.info("Thread was already running.")
