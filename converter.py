...
# MP3
def run_mp3(progress_filename, uploaded_file_path, mp3_encoding_type, mp3_bitrate, mp3_vbr_setting, output_path):
    if mp3_encoding_type == "cbr":
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a libmp3lame -b:a {mp3_bitrate}k {output_path}.mp3')
    elif mp3_encoding_type == "abr":
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a libmp3lame --abr 1 -b:a {mp3_bitrate}k {output_path}.mp3')
    elif mp3_encoding_type == "vbr": 
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a libmp3lame -q:a {mp3_vbr_setting} {output_path}.mp3')
...
def run_wav(progress_filename, uploaded_file_path, is_keep_video, wav_bit_depth, output_path):
    if is_keep_video == "yes":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a pcm_s{wav_bit_depth}le {output_path}.wav')
...
def run_opus(progress_filename, uploaded_file_path, opus_encoding_type, opus_vorbis_slider, opus_cbr_bitrate, output_path):
    if opus_encoding_type == "opus_vbr":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a libopus -vbr off -b:a {opus_vorbis_slider}k {output_path}.opus')
...
def run_vorbis(progress_filename, uploaded_file_path, vorbis_encoding, vorbis_quality, opus_vorbis_slider, output_path):
    if vorbis_encoding == "abr": # ABR
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a libvorbis -b:a {opus_vorbis_slider}k {output_path}.mka')
    elif vorbis_encoding == "vbr": # True VBR
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy libvorbis -q:a {vorbis_quality} {output_path}.mka')
...
def run_flac(progress_filename, uploaded_file_path, is_keep_video, flac_compression, output_path):
    if is_keep_video == "yes":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a flac -compression_level {flac_compression} {output_path}.flac')
...
def run_alac(progress_filename, uploaded_file_path, is_keep_video, output_path):
    if is_keep_video == "yes":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a alac {output_path}.m4a')
...
def run_ac3(progress_filename, uploaded_file_path, is_keep_video, ac3_bitrate, output_path):
    ...
    if is_keep_video == "yes":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a ac3 -b:a {ac3_bitrate}k {output_path}.ac3')
...
def run_dts(progress_filename, uploaded_file_path, is_keep_video, dts_bitrate, output_path):
    if is_keep_video == "yes":
        ...
        run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a dca -b:a {dts_bitrate}k -strict -2 {output_path}.dts')

# CAF
def run_caf(progress_filename, uploaded_file_path, output_path):
    run_ffmpeg(progress_filename, uploaded_file_path, f'-c:v copy -c:a alac {output_path}.caf')
