    ...
    if is_keep_video == "yes":
        ...
        if fdk_type == "fdk_cbr":
            if is_fdk_lowpass == "yes":
                run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:s copy -c:v copy -c:a libfdk_aac -cutoff {fdk_lowpass} -b:a {fdk_cbr}k {output_path}.{output_ext}')
            else:
                run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:s copy -c:v copy -c:a libfdk_aac -b:a {fdk_cbr}k {output_path}.{output_ext}')
        else: # VBR
            if is_fdk_lowpass == "yes":
                run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:s copy -c:v copy -c:a libfdk_aac -cutoff {fdk_lowpass} -vbr {fdk_vbr} {output_path}.{output_ext}')
            else:
                run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:s copy -c:v copy -c:a libfdk_aac -vbr {fdk_vbr} {output_path}.{output_ext}')
...
def run_wav(uploaded_file_path, is_keep_video, wav_bit_depth, output_path):
    if is_keep_video == "yes":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:s copy -c:v copy -c:a pcm_s{wav_bit_depth}le {output_path}.mkv')
    else:
...
def run_mp4(uploaded_file_path, mp4_encoding_mode, crf_value, output_path):
    if mp4_encoding_mode == "keep_codecs":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -c:v copy -c:a copy -f mp4 -movflags faststart {output_path}.mp4')
    elif mp4_encoding_mode == "keep_video_codec":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -c:v copy -c:a libfdk_aac -vbr 5 -f mp4 -movflags faststart {output_path}.mp4')
    elif mp4_encoding_mode == 'convert_video_keep_audio':
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -c:v libx264 -crf {crf_value} -c:a copy -f mp4 -movflags faststart {output_path}.mp4')
    else:
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -c:v libx264 -preset {mp4_encoding_mode} -crf {crf_value} -c:a libfdk_aac -vbr 5 -f mp4 -movflags faststart {output_path}.mp4')
...
def run_opus(uploaded_file_path, opus_encoding_type, opus_vorbis_slider, opus_cbr_bitrate, output_path):
    if opus_encoding_type == "opus_vbr":
        ffmpeg_pipe(uploaded_file_path, f'opusenc --comment Comment="Encoded using freeaudioconverter.net" --bitrate {opus_vorbis_slider} - {output_path}.opus')
    else: # CBR
        ffmpeg_pipe(uploaded_file_path, f'opusenc --comment Comment="Encoded using freeaudioconverter.net" --hard-cbr --bitrate {opus_cbr_bitrate} - {output_path}.opus')
...
def run_vorbis(uploaded_file_path, vorbis_encoding, vorbis_quality, opus_vorbis_slider, output_path):
    if vorbis_encoding == "vbr_bitrate": # VBR with nominal bitrate
        ffmpeg_pipe(uploaded_file_path, f'oggenc -c "Comment=Encoded using freeaudioconverter.net" -b {opus_vorbis_slider} - -o {output_path}.ogg')
...
def run_flac(uploaded_file_path, is_keep_video, flac_compression, output_path):
    if is_keep_video == "yes":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:v copy -c:s copy -c:v copy -c:a flac {output_path}.mkv')
    else:
...
def run_ac3(uploaded_file_path, is_keep_video, ac3_bitrate, output_path):
    ...
    if is_keep_video == "yes":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:v copy -c:s copy -c:a ac3 -b:a {ac3_bitrate}k {output_path}.{output_ext}')
    else:
...
def run_dts(uploaded_file_path, is_keep_video, dts_bitrate, output_path):
    if is_keep_video == "yes":
        run_ffmpeg(uploaded_file_path, f'-map 0:V? -map 0:a? -map 0:s? -c:v copy -c:s copy -c:a dca -b:a {dts_bitrate}k -strict -2 {output_path}.mkv')
    else:
