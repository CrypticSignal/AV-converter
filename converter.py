...
# Info logger
info_logger = setup_logger('info_logger', 'InfoLog.log')
# Error Logger
error_logger = setup_logger('error_logger', 'ErrorLog.log')
...
def run_mp3(chosen_file, mp3_encoding_type, cbr_abr_bitrate, mp3_vbr_setting, is_y_switch, output_name, is_downmix, output_path):
    try:
        if mp3_encoding_type == "cbr":  
            ffmpeg(chosen_file, f'-ac 2 -f wav - | lame -b {cbr_abr_bitrate} - {output_path}.mp3')
            logger.info(command)
            ...
            if is_y_switch == "no":
                ffmpeg(chosen_file, f'-ac 2 -f wav - | lame -V {mp3_vbr_setting} - {output_path}.mp3')
            else:
                ffmpeg(chosen_file, f'-ac 2 -f wav - | lame -Y -V {mp3_vbr_setting} - {output_path}.mp3')
    except Exception as error:
        error_logger.error(error)
    else:
        info_logger.info(f'{chosen_file} converted.')
# AC3
    ...
    try:
        if fdk_type == "fdk-cbr":
            if is_fdk_lowpass == "yes":
                ffmpeg(chosen_file, f'{is_downmix} -f wav - | sudo fdkaac --bandwidth {fdk_lowpass} -b {fdk_cbr} - -o {output_path}.m4a')

            else:
                ffmpeg(chosen_file, f'{is_downmix} -f wav - | sudo fdkaac -b {fdk_cbr} - -o {output_path}.m4a')
        else: # VBR
            if is_fdk_lowpass == "yes":
                ffmpeg(chosen_file, f'{is_downmix} -f wav - | sudo fdkaac --bandwidth {fdk_lowpass} --bitrate-mode {fdk_vbr} - -o {output_path}.m4a')
            else:
                ffmpeg(chosen_file, f'{is_downmix} -f wav - | sudo fdkaac --bitrate-mode {fdk_vbr} - -o {output_path}.m4a')
    except Exception as error:
        error_logger.error(error)
    else:
        info_logger.info(f'{chosen_file} converted.')
...
def run_opus(chosen_file, opus_encoding_type, slider_value, opus_cbr_bitrate, output_name, is_downmix, output_path):
    try:
        if opus_encoding_type == "opus-vbr": 
            ffmpeg(chosen_file, f'{is_downmix} -f wav - | opusenc --vbr {slider_value} - {output_path}.opus')
        else: # CBR
            ffmpeg(chosen_file, f'{is_downmix} -f wav - | opusenc --hard-cbr --bitrate {opus_cbr_bitrate} - {output_path}.opus')
    except Exception as error:
        error_logger.error(error)
    else:
        info_logger.info(f'{chosen_file} converted.')
...
def run_vorbis(chosen_file, vorbis_encoding, vorbis_quality, slider_value, output_name, is_downmix, output_path):
    try:
        if vorbis_encoding == "vbr_bitrate": # VBR with nominal bitrate
            ffmpeg(chosen_file, f'{is_downmix} -f wav - | oggenc -b {slider_value} - -o {output_path}.ogg')
        elif vorbis_encoding == "vbr_quality": # True VBR
            ffmpeg(chosen_file, f'{is_downmix} -f wav - | oggenc -q {vorbis_quality} - -o {output_path}.ogg')
    except Exception as error:
        error_logger.error(error)
    else:
        info_logger.info(f'{chosen_file} converted.')
